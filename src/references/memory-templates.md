...
All mentor memory is stored globally at `~/.config/opencode/mentor/` (default).
Configurable via `ctx.options.memoryPath` in `opencode.jsonc` — pour compatibilité Claude Code, utiliser `~/.claude/mentor` via `compatClaudePath: true` ou `memoryPath: "~/.claude/mentor"`. Le chemin effectif est injecté dans les skills via `{{memoryPath}}` (et `{{memoryPathFallback}}` si fallback activé).
...
```
~/.config/opencode/mentor/          # ou ~/.claude/mentor/ si compatClaudePath
├── dev-profile.md
├── quiz-log.md
└── topics/
    ├── async-await.md
