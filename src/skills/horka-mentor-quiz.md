...
description: >
  ...
  Reads from {{memoryPath}}/ to know what topics exist and what's due for review.
  ...
  Do NOT use if no topics have been covered yet (no files in {{memoryPath}}/topics/).
---
...
Tente d'appeler `context7_resolve-library-id` (alias `mcp_context7_resolve-library-id`) avec query "test".

> Note Opencode : serveur MCP `context7` déclaré automatiquement par le plugin (`https://mcp.context7.com/mcp`). Si un autre préfixe est exposé, utilise-le — le gate reste souple (voir ci-dessous).
...
> Tool utilitaire : `horka_mentor_progress` (`get_profile`, `list_topics`, `get_topic`, `get_quiz_log`, `check_proactive`) — interroge `{{memoryPath}}/` (filesystem source de vérité, miroir `ctx.storage`). Idéal pour scanner `topics/` et vérifier `next_review`.
...
2. **Lis** `{{memoryPath}}/dev-profile.md` pour la langue.

3. **Scanne** `{{memoryPath}}/topics/` — liste tous les fichiers.
...
1. **Scanne les topic files** dans `{{memoryPath}}/topics/` — lis la section Status de chaque fichier pour trouver `next_review` (source de verite = topic files, pas quiz-log)
...
1. Cherche le topic dans `{{memoryPath}}/topics/`
...
3. **Utilise Context7** (`context7_resolve-library-id` → `context7_query-docs`) pour construire des exemples avec du code a jour (sauf mode --no-context7)
...
Dans `{{memoryPath}}/topics/<concept>.md` :
...
## References
- Pédagogie : `{{referencesDir}}/pedagogy.md`
- Règles de progression : `{{referencesDir}}/level-up-rules.md`
- Templates mémoire : `{{referencesDir}}/memory-templates.md`
